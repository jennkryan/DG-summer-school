...
for nel in arange(Nx):
    ...
    #Form post-processed solution
        ...
        # Left boundary elements
            lambar = np.float(-(RS+0.5*ell)+(zmap[j]-xleft)/h)
            bdrycc = boundarypp(p,ell,RS,zEval[j],lambar)
            for kk in arange(2*RS+ell):
                for m in arange(p+1):
                    # one-sided
        ...
        # Right boundary elements
            lambar = np.float((RS+0.5*ell)+(zmap[j]-xright)/h)
            bdrycc = boundarypp(p,ell,RS,zEval[j],lambar)
            for kk in arange(2*RS+ell):
                for m in arange(p+1):
                    # one-sided
                    upost[j] = upost[j]+bdrycc[kk][m]*uhat[Nx-(2*RS+ell)+kk][m]
