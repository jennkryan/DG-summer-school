...
def getkernelcoeff(ell,RS,lambar):
    # Define matrix to determine kernel coefficients
        A=np.zeros((2*RS+1,2*RS+1))
        for m in np.arange(2*RS+1):
            for gam in np.arange(2*RS+1):
                component = 0.
                for n in np.arange(m+1):
                    jsum = 0.
                    jsum = sum((-1)**(j+ell-1)*binom(ell-1,j)*((j-0.5*(ell-2))**(ell+n)-(j-0.5*ell)**(ell+n)) for j in np.arange(ell))
                    component += binom(m,n)*(gam-RS+lambar)**(m-n)*factorial(n)/factorial(n+ell)*jsum
...
#        sumcoeff = sum(c[n] for n in np.arange(2*RS+1))
#        if np.abs(1.0-sumcoeff)>1.e-6:
#            print('PROBLEM:  Sum of coefficients IS NOT ONE:',sumcoeff)
