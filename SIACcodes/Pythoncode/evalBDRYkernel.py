...
#######################################################################                                                                                            
#  evaluate the kernel:
#  K(x) = sum_gam c_gam psi^{ell}(x-xgam),
#  where psi^{ell}(x-gam) is a B-spline of order ell centered at gam. 

def evalBDRYkernel(ell,RS,cgam,gpts,kerzeta,lambar):

    # Define B-spline breaks for a B-spline of order ell                                                                                                            
    bsbrks = np.linspace(-0.5*(2*RS+ell)+lambar,0.5*(2*RS+ell)+lambar,2*RS+ell+1)
