...
MeshOut = np.zeros((Nx,evalPoints+1))
ApproxOut = np.zeros((Nx,3))
for nel in np.arange(Nx):
    h = x_grid[nel+1] - x_grid[nel]
    ...
    MeshOut[nel][0] = int(nel+1)
    MeshOut[nel][1:evalPoints+1] = zmap[0:evalPoints]
    #Form L2-projection
    ApproxOut[nel][0] = int(nel+1)
    ...
    ApproxOut[nel][1:evalPoints+1] = uval[0:evalPoints]
                
    fApprox.extend(uval)
    
sourceFile1 = open('Mesh' + '__domain_'+str(xleft)+str(xright)+'__N_' + str(Nx) +'__deg_'+str(p) + '.txt','w')
print('dim = 1\n $Nodes \n', Nx, file = sourceFile1)
for nel in np.arange(Nx):
    print(int(MeshOut[nel][0]),end = ' ',file = sourceFile1)
    for j in np.arange(evalPoints):
        print(MeshOut[nel][j+1],end = ' ', file = sourceFile1)
    print(file = sourceFile1)

sourceFile2 = open('Nodes' + '__domain_'+str(xleft)+str(xright)+'__N_' + str(Nx) +'__deg_'+str(p) + '.txt','w')
print('dim = 1\n $Fields \n', Nx, p, 'Legendre',1,file = sourceFile2)
for nel in np.arange(Nx):
    print(int(ApproxOut[nel][0]),end = ' ',file = sourceFile2)
    for j in np.arange(evalPoints):
        print(ApproxOut[nel][j+1],end = ' ',file = sourceFile2)
    print(file = sourceFile2)
