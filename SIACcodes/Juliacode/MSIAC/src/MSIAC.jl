...
using LinearAlgebra, Einsum, PyCall, PyPlot, DelimitedFiles, Delaunay, Distributions, Printf,CircularArrays
using WriteVTK, VTKDataIO
import BSplines, FastGaussQuadrature, SpecialPolynomials, Polynomials
...
export filter_data, get_eL2, test_filter, GZW, in_polygon, evaluate_point, polynomial_degree
...
function filter_data(msh::MESH, fld::FIELD, kType::String; rx =(-1)::Int, lx=(-1)::Int, ry=(-1)::Int, ly=(-1)::Int,
	                 dumpFile="nofile"::String, theta="auto"::Union{String,dbl,Vector{dbl}}, scaling="auto"::Union{String,dbl,Vector{dbl}},
					 inexact=false::Bool, parallel=true::Bool)
	...
	filt = zeros(Qt,msh.N)
	if parallel
		Threads.@threads for e in 1:msh.N
			for i = 1:Qt
				zeta      = [fld.zw[1].nodes[i1[i]], fld.zw[2].nodes[i2[i]]]
				filt[i,e] = filter_convolution(msh,fld,e, zeta,kType,kx =[rx,lx,rx,lx,1], ky =[ry,ly,ry,ly,1],theta = theta, scaling = scaling, inexact=inexact)
			end
		end
	else
		for e in 1:msh.N
...
function line_convolution(msh::MESH,fld::FIELD, kernel::Vector{KERNEL}, eID::Int, zeta::vecD,
	...
	if track_footprint
		...
		for j = 1 : msh.N
			ca = ele_ca(msh.elmts[j].type)
			coords = element_coords(msh,j)
			af.plot(coords[1,ca], coords[2,ca], c = "k")
			cen = element_centre(msh,j)
			af.text(cen[1], cen[2], string(j))
		end
		for j = 1 : length(kB)
			af.scatter(kB[j].z[1], kB[j].z[2], c = "g", zorder = 4)
			j == length(kB)-1 && break
