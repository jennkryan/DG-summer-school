...
export update_phys!, update_modes!, update_fields!, load_data,load_field, create_analytic_field, load_modes, load_modes_file, get_exact_data, eval_analytic_field,
       plot_field,write_vtk, update_points!, get_evaluation_points, get_evaluation_weights, get_field
...
function eval_basis(basis::EXPANSION, z1::vecD, z2::vecD)
	...
	if basis.type == "pk" || basis.type == "Pk" || basis.type == "PK"
	    for j = 1:length(basis.m1)
	        ...
	        JYvals  = SP.Jacobi{2 * basis.m1[j] + 1, 0.0}(vcat(zeros(basis.m2[j]),1))
			JXvals  = SP.Jacobi{0.0, 0.0}(vcat(zeros(basis.m1[j]),1))
...
Parameters:
===========
basis: a polyBasis structure (expansion, mass matrix,etc)
zw1: points sampled along 1st dir
zw2: points sampled along 2nd dir
f: the projection function with size [length(zw1) * length(zw2), #elements]
Returns: modes matrix with dimension [data.degree + 1, #elements]
"""
...
function update_element_modes!(field::FIELD, zw::Vector{GZW}, e::Int)
	...
	if field.basis.type == "Pk" || field.basis.type == "pk" || field.basis.type == "Pk"
		field.modes[:,e] = [sum(f .* w12 .* phi[:, j] .* 0.5 .* (1 .- zw2.nodes[i2])) for j = 1:length(data.m1)]
	else
