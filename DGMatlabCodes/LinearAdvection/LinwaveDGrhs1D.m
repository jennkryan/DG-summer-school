function [rhsu] = LinwaveDGrhs1D(x,u,h,k,m,N,Ma,S,VtoE,wavesp)
...
fluxr = LinwaveLF(ue(2,2:N+1),ue(1,3:N+2),0,wavesp); 
fluxl = LinwaveLF(ue(2,1:N),ue(1,2:N+1),0,wavesp);

% Compute right hand side of Maxwell's equation
ru = wavesp*S'*u - (Imat(:,m+1)*fluxr(1,:) - Imat(:,1)*fluxl(1,:));
rhsu = (h/2*Ma)\ru;
return