function [u] = LinwaveDG1D(x,u,h,m,N,CFL,FinalTime,wavesp)
...
while (time < FinalTime)
  ...
  % Update solution
  rhsu  = LinwaveDGrhs1D(x,u,h,k,m,N,Ma,S,VtoE,wavesp); 
  u1 = u + k*rhsu; 
  %u1 = WENOlimitDG(x,u1,m,h,N,V,iV,Q,Xm,Xp);
  
  rhsu  = LinwaveDGrhs1D(x,u1,h,k,m,N,Ma,S,VtoE,wavesp);
  u2 = (3*u + u1 + k*rhsu)/4; 
  %u2 = WENOlimitDG(x,u2,m,h,N,V,iV,Q,Xm,Xp);

  rhsu  = LinwaveDGrhs1D(x,u2,h,k,m,N,Ma,S,VtoE,wavesp); 
  u = (u + 2*u2 + 2*k*rhsu)/3; 
